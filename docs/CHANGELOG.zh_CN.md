...
- 将 AB-731 原创四选一练习题由 18 道扩充至 100 道，并按三个官方技能领域
  分配为 40/38/22；错题记录同步支持全部 100 道，不再局限于前 32 道。
- 在每个 AB-731 页面底部加入清晰操作提示：长按 `OK` 返回 Passport 主页；
  长按 `UP` 清空已保存答题记录、重置当前练习并回到 AB-731 首页，同时显示确认。
- 修复进入 AB-731 及后续页面切换时，旧页面与新页面短暂共存导致 LVGL 固定
  24 KB 内存耗尽的问题；现在创建新页面前会先释放旧页面内容。短按并松开
  `OK` 也恢复为上游启动器的一次性进入语义。
- 恢复原项目的可穿戴设备省电行为：连续五分钟没有按键事件后，LCD 控制器、
  背光、电量计、按键 ADC 与主控分别进入对应的休眠状态；任意实体键均可唤醒。
  唤醒键松开前不接收操作，避免唤醒时误开应用。
- 蓝色首页背景上的文字全部改为高对比白色。
- 通过有界事件队列把导航与 LVGL 重绘从 ESP 定时器按键回调迁移到应用任务，
  避免按下确认键后定时器任务卡死并触发看门狗。
- 在 Passport 首页增加日期与时间栏；完全离线的固件以编译时间初始化软件时钟，
  每分钟自动更新，并避开头像、身份介绍、应用入口和电量显示。
- 将头像资源由面部方形裁切改为 72 × 96 的完整 3:4 人物图，保留头发、脸部、
  领巾和上半身，并同步扩大左侧头像显示区域。
- 按用户要求恢复“左头像、右身份”的上半区结构，下半区专门展示自己编写的
  AB-731 应用；身份和应用文字均直接挂在屏幕根节点，避免容器裁切。
- 再次重构首页为纵向居中布局：头像、姓名、身份、介绍和应用分区各自占用独立行，
  不再把文字放入嵌套身份卡，从根本上避免内边距与边框造成的裁切。
- 缩小“MY APPS”分区标题，并加宽身份卡、收紧内边距和介绍文字字号，确保头像、
  姓名、身份与分享说明在小屏上完整显示。
- 将 Passport 首页重构为单头像布局，重新设计姓名、身份文案和应用菜单层级；
  AB-731 默认高亮，按一次确认键即可打开，但不会在开机时自动进入。头像保留
  新的浅蓝像素天空背景，考试内长按确认键可返回个人主页。

- AB-731 答题页同时显示四个选项，上/下键只负责移动高亮选择。

- 将硬件演示菜单替换为离线 AB-731 随身练习应用：支持三键选项作答、即时解析、全部题目/错题重练、电量显示、单次得分和异步本地进度保存。新增 18 道对齐 Microsoft Learn 2026 年 7 月 22 日考纲的原创练习题，并为答题状态机增加主机端测试。
